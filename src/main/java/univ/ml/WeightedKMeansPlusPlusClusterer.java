...
public class WeightedKMeansPlusPlusClusterer<T extends Sample> extends Clusterer<T> {
     ...
     */
    public WeightedKMeansPlusPlusClusterer(final int k) {
        this(k, -1);
    }
     ...
     */
    public WeightedKMeansPlusPlusClusterer(final int k, final int maxIterations, final DistanceMeasure measure) {
    ...
    private Clusterable centroidOf(final Collection<T> points, final int dimension) {
        ...
        for (final T p : points) {
            ...
            for (int i = 0; i < centroid.length; i++) {
                centroid[i] += p.getWeight() * point[i];
            }
            overallWeight += p.getWeight();
        }
        for (int i = 0; i < centroid.length; i++) {
            centroid[i] /= overallWeight;
        }
